    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="w-full max-w-sm rounded-2xl border" style={{ background: "var(--bg-surface)", borderColor: "var(--border)", boxShadow: "var(--shadow)" }}>
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "var(--border-sub)" }}>
          <h3 className="font-bold text-base" style={{ color: "var(--text-primary)" }}>{isEdit ? "Edit Partner" : "Add New Partner"}</h3>
          <button onClick={onClose} style={{ color: "var(--text-muted)" }}>✕</button>
        </div>
        <form onSubmit={(e) => { e.preventDefault(); onSubmit({ name, note }); }} className="p-6 space-y-3">
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "var(--text-sec)" }}>Partner Name</label>
            <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Ramesh Gupta"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: "var(--accent)" }}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,.6)" }} onClick={(e) => e.target === e.currentTarget && onCancel()}>
      <div className="w-full max-w-sm rounded-2xl border p-5" style={{ background: "var(--bg-surface)", borderColor: "var(--border)", boxShadow: "var(--shadow)" }}>
        ...
        <p className="text-sm mb-5" style={{ color: "var(--text-sec)" }}>
          This permanently deletes the partner and every transaction logged against them. This can't be undone.
        </p>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="font-bold text-lg" style={{ color: "var(--text-primary)" }}>Partners</h2>
          <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>Money transferred to and received from each business partner</p>
        </div>
        <button onClick={() => { setEditingPartner(null); setShowForm(true); }} className="px-4 py-2.5 rounded-xl text-sm font-semibold text-white whitespace-nowrap" style={{ background: "var(--accent)" }}>
          + Add Partner
        </button>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => <div key={i} className="rounded-2xl border p-5 animate-pulse" style={{ background: "var(--bg-surface)", borderColor: "var(--border)", height: "200px" }} />)}
        </div>
      ) : partners.length === 0 ? (
        <div className="rounded-2xl border p-10 text-center" style={{ background: "var(--bg-surface)", borderColor: "var(--border)" }}>
          <p className="font-semibold" style={{ color: "var(--text-primary)" }}>No partners yet</p>
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Add your first partner to start tracking money transfers.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {partners.map((p) => {
            const positive = (p.netBalance || 0) >= 0;
            return (
              <div
                key={p._id}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {p.note && <p className="text-xs mt-0.5 truncate" style={{ color: "var(--text-muted)" }}>{p.note}</p>}
                  </div>
                  <span
                  ...
                  >
                    {positive ? "▲" : "▼"} ₹{fmt(Math.abs(p.netBalance || 0))}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-xl px-3 py-2" style={{ background: "var(--bg-elevated)" }}>
                    ...
                    <p className="text-sm font-bold mt-0.5 tabular-nums" style={{ color: "#22c55e" }}>₹{fmt(p.totalReceived)}</p>
                  </div>
                  <div className="rounded-xl px-3 py-2" style={{ background: "var(--bg-elevated)" }}>
                    <p className="text-[10px] uppercase tracking-wider font-semibold" style={{ color: "var(--text-muted)" }}>Transferred</p>
                    <p className="text-sm font-bold mt-0.5 tabular-nums" style={{ color: "var(--danger-text)" }}>₹{fmt(p.totalTransferred)}</p>
                  </div>
                </div>
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>{p.transactionCount} transactions · last {fmtDate(p.lastTransactionDate)}</p>

                <div className="flex gap-2 pt-2 border-t" style={{ borderColor: "var(--border-sub)" }} onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => navigate(`/superadmin/partners/${p._id}`)} className="flex-1 text-xs px-3 py-2 rounded-lg font-semibold text-white" style={{ background: "var(--accent)" }}>View Details →</button>
                  <button onClick={() => { setEditingPartner(p); setShowForm(true); }} className="text-xs px-3 py-2 rounded-lg border font-medium" style={{ borderColor: "var(--border)", color: "var(--text-sec)", background: "var(--bg-elevated)" }}>Edit</button>
                  <button onClick={() => setDeleteTarget(p)} className="text-xs px-3 py-2 rounded-lg border font-medium" style={{ borderColor: "var(--danger-border)", color: "var(--danger-text)", background: "var(--danger-soft)" }}>Del</button>
                </div>
              </div>
            );
          })}
      ...
      {deleteTarget && <DeletePartnerModal partner={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={handleDelete} />}
    </div>
  );
}